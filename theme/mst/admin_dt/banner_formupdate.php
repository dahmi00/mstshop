...
@mkdir(G5_DATA_PATH."/banner_dt", G5_DIR_PERMISSION);
@chmod(G5_DATA_PATH."/banner_dt", G5_DIR_PERMISSION);
...
if ($bn_bimg_del)  @unlink(G5_DATA_PATH."/banner_dt/$bn_id");
...
if ($w=="")
{
    ...
    sql_query(" alter table {$g5['g5_shop_banner_table']}_daontheme auto_increment=1 ");

    $sql = " insert into {$g5['g5_shop_banner_table']}_daontheme
                set bn_alt        = '$bn_alt',
                    bn_url        = '$bn_url',
                    bn_device     = '$bn_device',
...
{
    $sql = " update {$g5['g5_shop_banner_table']}_daontheme
                set bn_alt        = '$bn_alt',
                    bn_url        = '$bn_url',
                    bn_device     = '$bn_device',
                    bn_position   = '$bn_position',
...
{
    @unlink(G5_DATA_PATH."/banner_dt/$bn_id");

    $sql = " delete from {$g5['g5_shop_banner_table']}_daontheme where bn_id = $bn_id ";
...
if ($w == "" || $w == "u")
{
    if ($_FILES['bn_bimg']['name']) upload_file($_FILES['bn_bimg']['tmp_name'], $bn_id, G5_DATA_PATH."/banner_dt");

    goto_url("./banner_form.php?w=u&amp;bn_id=$bn_id");
} else {
    goto_url("./banner_list.php?bn_position=$bn_positiona&amp;bn_device=$bn_devicea&amp;bn_time=$bn_timea&amp;page=$pagea");
}